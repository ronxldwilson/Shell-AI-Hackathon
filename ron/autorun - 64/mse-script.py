...
class Trainer:
    ...
    def validate(self):
        ...
        mse_orig = np.mean((preds_all_orig - targets_all_orig) ** 2)

        return total_loss / len(self.val_loader), mse_orig
    ...
    def train(self):
        ...
        for epoch in range(self.config.EPOCHS):
            ...
            print(f"Epoch {epoch+1}/{self.config.EPOCHS} | Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f} | Val MSE (orig): {val_mse:.4f}")
        ...
        if best_model_state is not None:
            ...
            print(f"✅ Loaded best model with Val MSE (orig): {best_mse:.4f}")
