...
class Config:
    ...
    K_ENSEMBLE = 64  # Increased from 32
    RANDOM_STATE = 42
...
class Trainer:
    ...
    def train_epoch(self, epoch):
        ...
        for X, y in tqdm(self.train_loader, desc=f"Epoch {epoch+1}/{self.config.EPOCHS}"):
            ...
            preds = self.model(X)  # Shape: (K_ENSEMBLE, batch_size, n_targets)
            
            # Debug print for first batch
            if epoch == 0 and total_loss == 0:
                print(f"Debug - preds shape: {preds.shape}, y shape: {y.shape}")
            
            # Aggregate ensemble predictions - mean across ensemble dimension (dim=1)
            preds_mean = torch.mean(preds, dim=1)  # Shape: (batch_size, n_targets)
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for X, y in self.val_loader:
                X, y = X.to(self.device), y.to(self.device)
                preds = self.model(X)  # Shape: (batch_size, K_ENSEMBLE, n_targets)
    ...
    def train(self):
        ...
        for epoch in range(self.config.EPOCHS):
            ...
            if val_mape < best_mape - self.config.MIN_DELTA:  # Significant improvement
                best_mape = val_mape
            ...
            if patience_counter >= self.config.PATIENCE:  # Early stopping
                print(f"⏹️  Early stopping at epoch {epoch+1} (no improvement for {self.config.PATIENCE} epochs)")
                break
...
if __name__ == '__main__':
    ...
    try:
        while True:
            ...
            print(f"✅ Finished run #{run_num} with MAPE: {best_val_mape:.2f}%\n")
            run_num += 1
