...
class Trainer:
    ...
    def train_xgboost(self):
        ...
        # We train one XGBRegressor per target column
        ...
        xgb_train_preds = np.column_stack([model.predict(self.X_train) for model in self.xgb_models])
        xgb_val_preds = np.column_stack([model.predict(self.X_val) for model in self.xgb_models])
        xgb_test_preds = np.column_stack([model.predict(self.X_test) for model in self.xgb_models])

        # Add XGBoost predictions as new features
        self.X_train = np.concatenate([self.X_train, xgb_train_preds], axis=1)
        self.X_val = np.concatenate([self.X_val, xgb_val_preds], axis=1)
        self.X_test = np.concatenate([self.X_test, xgb_test_preds], axis=1)
    ...
    def predict(self, submission_path=None):
        self.model.eval()
        preds_all = []
...
if __name__ == '__main__':
    ...
    try:
        while True:
            ...
            trainer.predict(submission_file)
